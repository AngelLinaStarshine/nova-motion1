    ...
    <section id="membership" style={{ background: "var(--color-bg-warm)", padding: "110px 40px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <h2 className="display-heading" style={{ fontSize: "clamp(36px, 4vw, 56px)", marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24, alignItems: "start" }}>
          {MEMBERSHIPS.map((plan) => (
            ...
            >
              {plan.popular && (
                <div style={{ position: "absolute", top: -14, left: "50%", transform: "translateX(-50%)", background: "var(--color-gold)", color: "#fff", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", padding: "5px 18px", whiteSpace: "nowrap" }}>
                  Most Popular
                </div>
              )}

              {/* Plan name */}
              <div style={{ fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase", color: plan.popular ? "var(--color-gold)" : "var(--color-text-faint)", marginBottom: 12 }}>
                {plan.name}
              </div>

              {/* Price */}
              <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 6 }}>
                <span style={{ fontFamily: "var(--font-display)", fontSize: 52, fontWeight: 300, color: plan.popular ? "var(--color-text-inv)" : "var(--color-text)" }}>
                  ${plan.price}
                </span>
                <span style={{ fontSize: 13, color: plan.popular ? "#8c857e" : "var(--color-text-faint)" }}>/mo</span>
              </div>
              <p style={{ fontSize: 13, color: plan.popular ? "#8c857e" : "var(--color-text-faint)", marginBottom: 28 }}>{plan.tagline}</p>
                ...
                {plan.features.map((f) => (
                  <div key={f} style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 14 }}>
                    <div style={{ width: 18, height: 18, borderRadius: "50%", background: plan.popular ? "var(--color-gold)" : "var(--color-border)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>
                      <span style={{ fontSize: 9, color: plan.popular ? "#fff" : "var(--color-text-faint)", lineHeight: 1 }}>✓</span>
                    </div>
                    <span style={{ fontSize: 14, color: plan.popular ? "#c8bfb0" : "var(--color-text-muted)", fontWeight: 300 }}>{f}</span>
                  </div>
                ))}
              </div>
